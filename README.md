...
\dim{\mathbb{S}} =
\begin{cases}
12(n + 1) - 7, & \text{for continuous action space}, \\
12(n + 1), & \text{for meta-action space}.
\end{cases}
$$
...
The `/baseline` folder contains the code that was entirely generated by the GPT-like tools and has not been verified properly. The authors of the repository take no responsibility for the code mentioned there.

The folder has been preserved for the educational purpose only.
